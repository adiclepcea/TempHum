 ...
 */
#define TX_START() UCSRB |= _BV(TXEN) // Enable TX
#define TX_STOP() UCSRB &= ~_BV(TXEN) // Disable TX
#define RX_START() UCSRB |= _BV(RXEN) // Enable RX
#define RX_STOP() UCSRB &= ~_BV(RXEN) // Disable RX
#define COMM_START() TX_START(); RX_START() // Enable communications
...
/* 
#define RX_INTEN() UCSRB |= _BV(RXCIE) // Enable interrupt on RX complete
#define RX_INTDIS() UCSRB &= ~_BV(RXCIE) // Disable RX interrupt
#define TX_INTEN() UCSRB |= _BV(TXCIE) // Enable interrupt on TX complete
#define TX_INTDIS() UCSRB &= ~_BV(TXCIE) // Disable TX interrupt
*/
/* Prototypes */
void initUART(void);
